...
lot fixe sous le mode strict — corrigé. Détail en
[§14](#la-fiche-regardée-avec-le-même-œil--15-septembre-2026).

**Le même jour, en fin de matinée** — **le coup d'œil sur les couches air
et radar est fait**, la réserve de J9 est soldée : champ PM2,5 national aux
couleurs de la légende, 18 frames radar au pas de cinq minutes, cellules
dessinées au zoom 9 (§2). En lisant `/statut` au passage, trois sources
lisaient en retard **sans panne** — `arome`, `cams`, `prefectures` — parce
que le registre attendait une cadence de donnée que la lecture ne promet
...
(47ᵉ migration) ; l'accueil et la carte disent les mêmes nombres. **Tranché
l'après-midi ([ADR-027](adr/027-detections-hors-perimetre.md))** : les
détections hors périmètre restent importées et ne sont jamais publiées —
2 914 événements sur 4 769 masqués au rattrapage, 61 % de tout ce qui a été
regroupé depuis août. L'après-midi a aussi donné un état de source qui
manquait — l'**import manuel**, pour IGN et EFFIS, « 9/9 sources en
service » — et la **page commune** lit enfin ses événements. Le VPS attend
...
registre — **corrigée le même matin** avec celles de `cams` et de
`prefectures` (migration `registre_cadence_de_lecture`, dette traitée au
§15) : les trois lisent « à jour » depuis 10 h 14.
...
#### Le périmètre du service est appliqué — 15 septembre 2026, après-midi

- ✅ **Les détections hors périmètre sont importées, jamais publiées**
  ([ADR-027](adr/027-detections-hors-perimetre.md), 49ᵉ migration
  `events_hors_perimetre`, appliquée en production et rejouée sans effet).
  La règle est celle des agrégats — dans un département ou à moins de
  0,05° de l'un d'eux —, en un seul endroit (`fire.point_in_territory`) ;
  `fire.events.in_territory` porte le verdict, posé par déclencheur à
  chaque écriture ; un événement hors périmètre est masqué par le
  mécanisme existant (`hidden`, motif « hors périmètre »), que toutes les
  fonctions publiques honorent déjà, et le déclencheur le force quel que
  soit le chemin — regroupement, cycle de vie, retouche. **Mesuré au
  rattrapage : 2 914 événements sur 4 769 masqués — 61 % de tout ce qui a
  été regroupé depuis août** ; tous à plus de cinq kilomètres d'un
  département, la distribution des distances commence à 5 km. « Ersa »,
  34 observations, était dans la plaine du Pô à 239 km de la Corse ;
  « Hendaye » en Castille à 250 km ; « Condé-sur-l'Escaut » à 63 km en
  Belgique. La fiche d'un tel identifiant dit **« Hors du périmètre du
  service »** plutôt qu'un 404 muet. Vérifié : catalogue, carte (306
  événements sur 7 jours, 72 départements) et agrégats disent les mêmes
  nombres ; aucune erreur de console. L'ingestion ne change pas — rien
  n'est détruit, la décision est réversible d'un `update` ; le volume en
  base reste à revoir avec la purge de rétention (J5)

#### Le poste ingère sous `mapfeux_ingest` — 15 septembre 2026, soir

- ✅ **Le rôle d'ingestion, sur le poste aussi.** Mot de passe de
  `mapfeux_ingest` tourné, `DATABASE_URL` du worker passée à ce rôle, la
  chaîne `postgres` reléguée sous `ADMIN_DATABASE_URL` pour les migrations,
  secret CI réaligné en forme pooler — les runners GitHub ne joignent pas
  l'hôte direct. Cinq tâches relancées à 16 h 07 : neuf passes en succès
  sous le rôle (`import_runs` relu sous ce même rôle). **Trouvé avant de
  basculer** : le rôle n'a aucun droit sur `app.territories`, et le
  déclencheur de périmètre de l'après-midi lisait cette table sous le rôle
  qui écrit — sous `mapfeux_ingest`, chaque écriture d'événement aurait
  échoué, le regroupement entier à la première passe ; sous `postgres`, le
  poste ne l'a jamais vu. 50ᵉ migration : `fire.point_in_territory` en
...
| L'ingestion FIRMS importe hors de France | La zone d'import est un rectangle qui déborde des frontières : sur les sept jours au 15 septembre, **716 événements dans la zone, 322 en France** — le reste en Espagne, en Belgique, en Allemagne, en Italie ou en mer. Chaque événement reçoit la commune française la plus proche, quelle que soit la distance : `MPF-VHR8YJ85` « près de Condé-sur-l'Escaut » est un site industriel belge près de Gand, à quelque 80 km. Les agrégats départementaux n'en comptent plus aucun (47ᵉ migration), mais la carte à l'échelle d'une zone, le catalogue et les fiches les servent toujours. Le périmètre du service est « France métropolitaine et Corse » (§2.4, FR-001) ; il n'était pas appliqué à l'ingestion. **Tranché le 15 septembre, [ADR-027](adr/027-detections-hors-perimetre.md)** : importées, jamais publiées — masquées en base par déclencheur, 2 914 événements sur 4 769 au rattrapage (61 %). Reste à revoir avec la purge de rétention (J5) : la base garde une détection sur deux hors périmètre | Traité |
| Le poste Windows ingère sous le rôle `postgres` | Vu le 15 septembre en appliquant la 45ᵉ migration par la chaîne du worker : la connexion s'ouvrait en `postgres`, les sept tâches tournaient avec le superutilisateur. **Traité le même soir** : mot de passe de `mapfeux_ingest` tourné, `DATABASE_URL` du `.env` du worker passée à ce rôle — la chaîne `postgres` reste sous `ADMIN_DATABASE_URL`, pour les migrations seulement —, secret CI `INGESTION_DATABASE_URL` réaligné en forme pooler, cinq tâches relancées : neuf passes en succès sous `mapfeux_ingest`. **Trouvé en préparant la bascule** : le rôle n'a aucun droit sur `app.territories`, et le déclencheur de périmètre (49ᵉ) lisait cette table sous le rôle qui écrit — chaque écriture d'événement aurait échoué en « permission denied » ; 50ᵉ migration, `fire.point_in_territory` en `security definer`. Le rôle garde ses vingt tables, rien de plus | Traité |
